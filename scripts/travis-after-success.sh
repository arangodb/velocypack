...
if ${COVERAGE:-false}; then
  ...
  # clear counters
  lcov --directory "$base_dir" --capture --initial --output-file base_coverage.info || ferr "failed lcov"
  lcov --directory "$base_dir" --zerocounters || ferr "failed lcov"

  # run tests
  (cd tests && ctest -V) || ferr "failed to run tests"

  # collect coverage info
  lcov --directory "$base_dir" --capture --output-file test_coverage.info || ferr "failed lcov"
