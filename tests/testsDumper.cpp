...
TEST(StringDumperTest, CustomWithCallbackDefaultHandler) {
  ...
  *p = 0xf3;
  ...
  Dumper dumper(&sink, &options);
  ASSERT_VELOCYPACK_EXCEPTION(dumper.dump(b.slice()),
                              Exception::NotImplemented);

  ASSERT_VELOCYPACK_EXCEPTION(handler.dump(b.slice(), &dumper, b.slice()),
                              Exception::NotImplemented);
  ASSERT_VELOCYPACK_EXCEPTION(handler.toString(b.slice(), nullptr, b.slice()),
                              Exception::NotImplemented);
}
