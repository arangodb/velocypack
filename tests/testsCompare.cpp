...
TEST(NormalizedCompareTest, MinKey) {
  ASSERT_TRUE(
      NormalizedCompare::equals(Slice::minKeySlice(), Slice::minKeySlice()));

  ASSERT_FALSE(
      NormalizedCompare::equals(Slice::minKeySlice(), Slice::maxKeySlice()));
  ASSERT_FALSE(NormalizedCompare::equals(Slice::minKeySlice(),
                                         Parser::fromJson("null")->slice()));
}

TEST(NormalizedCompareTest, MaxKey) {
  ASSERT_TRUE(
      NormalizedCompare::equals(Slice::maxKeySlice(), Slice::maxKeySlice()));

  ASSERT_FALSE(
      NormalizedCompare::equals(Slice::maxKeySlice(), Slice::minKeySlice()));
  ASSERT_FALSE(NormalizedCompare::equals(Slice::maxKeySlice(),
                                         Parser::fromJson("null")->slice()));
}
...
TEST(NormalizedCompareTest, Custom) {
  ...
  *p++ = 0xaa;
  ...
  ASSERT_VELOCYPACK_EXCEPTION(NormalizedCompare::equals(b.slice(), b.slice()),
